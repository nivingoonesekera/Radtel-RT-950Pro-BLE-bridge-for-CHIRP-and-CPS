...
;    NO PrependPath (your PATH / default `python` is left alone), and it's a
;    version-specific install so other Python versions are untouched. Reached
;    only via the `py -3.10` launcher (Include_launcher) thereafter.
Filename: "{tmp}\{#PyExe}"; \
  Parameters: "/quiet InstallAllUsers=1 Include_launcher=1 Include_pip=1"; \
  StatusMsg: "Installing Python {#PyVer} ..."; Flags: waituntilterminated; Check: NeedPython

; 2) bleak + pyserial into that 3.10 (the py launcher always lands in {win}).
;    No --upgrade: already-installed copies are left at their current version.
...
begin
  if CurPageID = wpReady then begin
    ...
    // Nothing to fetch (both already present) -> skip the download page.
    if NeedPython or NeedCom0com then begin
      DownloadPage.Show;
      try
        try
